...
- **UI:** campanita → **Ver rubros** abre pop-up gráfico (Chart.js barras horizontales + leyenda COICOP). API: `GET /api/ipc/series?period=YYYY-MM`. Las notificaciones `NEW_IPC` guardan `metadata.series`.
